...
public class ReviewLaterOperation extends BaseOperation implements IDBOperation {

	private final LexEntry entry;

	public ReviewLaterOperation(PgEnvironment pgEnvironment, Language language, LexEntry lexEntry) {
		this.entry = lexEntry;
	...
	public void execute() throws OperationRejectedException {
		...
		try {
			Database.getInstance(DbSelector.getDbNameByLanguage(pgEnvironment, language)).reviewLater(entry);
